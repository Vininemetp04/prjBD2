...
--View de Produtos com Categoria e Estoque
...
FROM Funcionario f
JOIN Funcionario_Setor fs
  ON f.func_id = fs.func_id
JOIN Setor s
  ON fs.setor_id = s.setor_id;
...
SELECT
  ...
  f.func_nome                      AS NomeFuncionario
FROM Venda v
LEFT JOIN vw_Total_Itens_Por_Venda t
  ON v.venda_id = t.VendaID
JOIN Cliente cli
  ON v.cli_id = cli.cli_id
JOIN Funcionario f
  ON v.func_id = f.func_id;
...
FROM Compra c
LEFT JOIN vw_Total_Itens_Por_Compra t
  ON c.comp_id = t.CompraID
JOIN Fornecedor f
  ON c.forn_id = f.forn_id
JOIN Funcionario fn
  ON c.func_id = fn.func_id;
...
WHERE
  DATE('now') BETWEEN pr.promo_inicio AND pr.promo_fim;
